...
class AmenityService {
    ...
    async getAmenitiesStatisticsbyBranch(branchId = null) {
        try {
            ...
            const [result] = await database.query(QUERY, branchId ? [branchId] : []);
    ...
    async addAmenityToRooms(amenityId, roomIds) {
        try {
            // Tạo danh sách các cặp (MaPhong, MaTienNghi) để insert vào bảng TienNghiPhong_Phong
            const values = roomIds.map((roomId) => [roomId, amenityId]);
